...
export class ReviewService {
  private apiUrl = `${environment.apiUrl}/api`; // Adjusted to base API URL
  ...
  deleteReview(reviewId: number): Observable<void> {
    const url = `${this.apiUrl}/Review/${reviewId}`;
    console.log(`Deleting review ${reviewId} at: ${url}`);
    return this.http.delete<void>(url, { headers: this.getAuthHeaders() }).pipe(
      catchError((err) => {
        console.error(`Error deleting review ${reviewId}:`, err);
        return throwError(() => new Error(err.error?.message || `Failed to delete review with ID ${reviewId}`));
      })
    );
  }
}